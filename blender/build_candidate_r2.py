"""Round 2 Candidate Repair: Clean palm cut, connected palm mesh, proper thumb weights.

Input: blender/hand_gf0021.blend (Hour 3 whole mesh base)
Output: blender/candidates/round2/hand_gf0021.candidate_r2.blend
Render output: blender/candidates/round2/demos/{name}_front.png

Hypothesis:
1. Deleting finger vertices cleanly at z > 0.096 & x < 0.032 while preserving the thumb-index web
   yields a single connected palm component without isolating the thenar margin (comp69) or web (comp48).
2. For thumb vertices (x > 0.015, z < 0.08), attenuating wrist bone influence and reweighting thumb.CMC/MCP
   prevents the thumb sheet from flattening and pinning to the wrist during pose.
3. Rigid finger phalanges inserted with 0.012m overlap into palm eliminate MCP gaps across all 8 letters.
"""
...
HOUR3_BLEND = ROOT / "hand_gf0021.blend"
OUT_DIR = ROOT / "candidates" / "round2"
OUT_DIR.mkdir(parents=True, exist_ok=True)
OUT_BLEND = OUT_DIR / "hand_gf0021.candidate_r2.blend"
...
def clean_strip_fingers(hand):
    ...
    # Precise removal of four finger rays (index/middle/ring/pinky)
    # We remove vertices with z > 0.096 and x < 0.030, which cleanly clips all 4 finger bases
    # while preserving the full thumb and palm web.
    dead = [v for v in bm.verts if v.co.z > 0.096 and v.co.x < 0.030]
    print(f"clean_strip_fingers: removing {len(dead)} verts of {len(bm.verts)}")
...
def rebind_palm_and_thumb(hand, arm):
    ...
    for vi, vert in enumerate(hand.data.vertices):
        p = mw @ vert.co
        is_thumb_zone = (p.x > 0.015 and p.z < 0.085 and p.y < 0.005)
        raw = []
        for name, a, c, blen in segs:
            ...
            d = (p - (a + ab * t)).length
            w = math.exp(-((d / (blen * 0.50)) ** 2))
            if name in finger_bones:
                w *= 0.005
            # In thumb zone, don't let wrist pin down the thumb flesh
            if is_thumb_zone and name == "wrist":
                w *= 0.05
            # Boost thumb bone response in thumb zone
            if is_thumb_zone and name in thumb_bones:
                w *= 2.5
            raw.append((name, w))
...
def render_all(scene, arm, cam, out_dir):
    ...
    for name in MAIN_LETTERS:
        ...
        print(f"Candidate R2 rendered: {path}")
        if cam is not None:
            cam.location = home
...
def main():
    if not HOUR3_BLEND.exists():
        raise SystemExit(f"missing {HOUR3_BLEND}")
    bpy.ops.wm.open_mainfile(filepath=str(HOUR3_BLEND))
    ...
    # Remove any existing hybrid finger segments
    ...
    clean_strip_fingers(hand)
    rebind_palm_and_thumb(hand, arm)
    ...
    print("Candidate R2 saved blend:", OUT_BLEND, "palm verts:", len(hand.data.vertices))

    # Render candidate stills
    render_all(scene, arm, cam, OUT_DEMOS)
    print("Candidate R2 completed successfully.")

"""Hour 4: HBM palm + independent finger phalanges.

Curl no longer twists the palm sheet. U stays index+middle extended, ring+pinky curled.
Does not move the camera. Does not change letters.json.
"""
...
def phalanx(name, p0, p1, r0, r1):
    ...
    pole0 = bm.verts.new(p0 - z_a * r0 * 0.15)
    rings = [
        ring(0.0, r0, r0 * 0.72),
        ring(length * 0.5, (r0 + r1) * 0.5, (r0 + r1) * 0.36),
        ring(length, r1, r1 * 0.70),
    ]
    pole1 = bm.verts.new(p1 + z_a * r1 * 0.18)
    for k in range(n_around):
        k2 = (k + 1) % n_around
...
def strip_hbm_fingers(hand):
    """Keep palm + thumb. Remove index/middle/ring/pinky from the sheet."""
    bpy.ops.object.select_all(action="DESELECT")
    ...
    dead = [v for v in bm.verts if v.co.z > 0.092 and v.co.x < 0.032]
    print("strip finger verts", len(dead), "of", len(bm.verts))
...
def main():
    ...
    strip_hbm_fingers(hand)
    rebind_palm(hand, arm)

    radii = {
        "index": (0.0090, 0.0076, 0.0062),
        "middle": (0.0094, 0.0078, 0.0064),
    ...
    for name, (r0, r1, r2) in radii.items():
        ...
        bind_rigid(phalanx(f"{name}_ph1", mcp_h, mcp_t, r0, r1), arm, f"{name}.MCP", mat)
    ...
    print("hour4 hybrid saved", "palm verts", len(hand.data.vertices))
